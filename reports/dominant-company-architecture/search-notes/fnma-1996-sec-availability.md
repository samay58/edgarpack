...
Fannie Mae's 1998 primary issuer Information Statement at `https://www.fanniemae.com/syndicated/documents/mbs/debtmarketing/DEBENTUR/W21931s.htm` describes the charter-based board structure and incorporates the 1997 proxy for stock-ownership details, but it is not SEC/EdgarPack/raw SEC evidence.

The control row is therefore marked `unresolved` and the evidence ledger records an `availability_exception` rather than treating the issuer page as normal control evidence.

"""Chinese prospectus section detection for STAR Market (CSRC-mandated structure)."""
...
# Canonical section map: Chinese title keyword -> (english slug, english title)
_CANONICAL_SECTIONS: dict[str, tuple[str, str]] = {
...
def _slug_for_title(title: str, section_num: int | None) -> tuple[str, str]:
    ...
    # Check canonical map
    for keyword, (slug, en_title) in _CANONICAL_SECTIONS.items():
        if keyword in title_clean:
            return slug, en_title

    # Fallback: use pypinyin for romanization
    prefix = f"ipo_s{section_num:02d}" if section_num else "ipo_other"
    try:
...
def find_sections_cn(markdown: str) -> list[Section]:
    """Find sections in a Chinese STAR Market prospectus.

    Args:
        markdown: Full markdown text of the prospectus.
    ...
    for m in _DECLARATIONS_PATTERN.finditer(markdown):
        slug, en_title = _CANONICAL_SECTIONS["重要声明"]
    ...
    for m in _SECTION_PATTERN.finditer(markdown):
        ...
        slug, en_title = _slug_for_title(title, section_num)
    ...
    if not matches:
        return [
            Section(
                ...
                warnings=["No section headings detected in Chinese prospectus"],
            )
        ]

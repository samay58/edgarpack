---
name: learn-pack
description: Generate a deep, narrative learning pack (docs/learn/) for a codebase. Two-phase workflow, discovery plus plan, then fill one piece at a time. Used when a developer or agent needs to truly understand a system, not just read its API. Trails tell stories; refs are the dictionary.
---
...
3. Open with the command the reader runs and the output it returns, then explain underneath. The trail should be followable start to finish without the reader opening the source.
4. Describe what the code does and cite `path/to/file.py:LINE`; do not paste function bodies. Keep runnable commands and small output samples, not implementation source. Collect the code references into one list at the end of the trail, not scattered through the prose.
5. Write in the user's voice (concrete, second-person, present tense). "You run X. Module Y receives the call." Not "The system processes the request through a pipeline of stages."
6. Stop after one trail. Tell the user it's ready. Wait for review.
...
```
<skill dir>/                  # this skill (.learn-pack/ in a repo, or ~/.agents/skills/learn-pack/ when global)
├── SKILL.md                  # This file
...
│   ├── install.sh            # Wires up Claude Code and Codex CLI (per-repo)
│   └── sync-global.sh        # Pushes this copy to the universal skill home
├── templates/
│   ├── README.md.tmpl
│   ├── trail.md.tmpl
│   ├── ref.md.tmpl
│   └── manifest.yml.tmpl
└── examples/
    ├── edgarpack/            # Primary worked reference (run-first trails)
    └── marginalia/           # Secondary worked reference (smaller pack)
...
You write to `docs/learn/`. You never write into this skill's own directory while filling.

## Bootstrapping a fresh run

Create `docs/learn/` in the host repo. The skill ships a tiny `scripts/scaffold.sh` that does only this, the directory plus an empty manifest stub; run it from the skill directory, or just create the folder yourself. The interesting work is yours.

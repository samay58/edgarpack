...
- Phase 3: WAVE A + HK MERGED 2026-07-05 on branch `streamline/phase3-build`. Entry gate (25-filer sweep) passed at 21/25 yielding, 0 wrong. Ten packets integrated (sonnet x8, opus x2): sweep-fixes(+2), s1-structure split, english-surface bilingual rendering, hk-extract-fixes (year-header anchor + garble gate), build-hk-acquire + build-hk-construct (`_COMPANY_META` deleted, metadata from filing), one-command-china (cold-start auto-build), dual-listing-adr + starter-universe (~53 verified names, venue routing, ADR resolution), harvest-china-sse. Gate green: 1,856 offline tests, ruff, format, mypy strict. Offline acceptance verified: BABA/BYDDY dual-listing resolution and the `--venue sec` teaching error render as designed. Remaining before Phase 4: adversarial review layer, then the live cold-build acceptance (`query BYD revenue`, needs network) at the Phase 4 readback.

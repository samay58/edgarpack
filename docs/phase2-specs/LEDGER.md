...
- 2026-07-05 Phase 3 post-review snapshot (after Wave A + HK + adversarial review + inline review fixes). Transcript totals: claude-fable-5 957,145; claude-opus-4-8 862,805; claude-sonnet-5 1,345,793. Phase 3 deltas: fable +279,274; opus +621,182; sonnet +998,360 (total 1,898,816 output). Structure held: every packet's implementation ran on routed sonnet/opus subagents; the orchestrator authored specs, merged, hand-resolved one cli.py conflict, and applied 6 small inline review fixes. CAVEAT on the clean split: the user switched the session model to Opus 4.8 mid-phase (via /model), so orchestrator main-loop output AFTER the switch bills as claude-opus-4-8, not claude-fable-5, and the opus delta also contains the adversarial-review subagents (5 refuters + 9 verifiers, correctly opus for review). So the raw fable-tag share (279k / 1.90M = 14.7%) UNDERCOUNTS orchestrator cost and the opus tag OVERCOUNTS it; the tag-based split is not cleanly measurable for Phase 3. What is measurable and true: all rank-and-file implementation went down-ladder (sonnet 998k dominates execution), and the orchestrator wrote no packet code, only specs + integration + 6 targeted fixes. Gate at every merge: green (1,860 offline tests, ruff, mypy strict). Adversarial layer: 9 findings, 6 confirmed (2 trust-critical value bugs, 1 venue-routing P1, 1 P2, 2 P3), 3 refuted; all confirmed fixed with regression tests; registration split (incl. the inline picker unification) drew zero findings.
